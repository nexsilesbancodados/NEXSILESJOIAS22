...
async function processMercadoPago(supabase: any, payload: any, query: Record<string, string>) {
  ...
  if (brevoKey) {
    const html = `<div style="font-family:Arial;max-width:600px;margin:0 auto;padding:20px"><h1>🎉 Pagamento Confirmado!</h1><p>Use o código abaixo para criar sua conta:</p><div style="background:#fef3c7;border:2px dashed #f59e0b;border-radius:8px;padding:20px;text-align:center;margin:20px 0"><div style="font-size:32px;font-weight:bold;color:#92400e;letter-spacing:4px;font-family:monospace">${codigoData}</div><p style="margin-top:10px;color:#666;font-size:12px">Válido por 30 dias</p></div><a href="https://nexsiles.com.br/auth" style="display:inline-block;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:bold">Criar Minha Conta</a></div>`;
    try {
