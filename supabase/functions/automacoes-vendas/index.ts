...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const venda of vendasRecentes || []) {
      ...
      // Check if we already sent post-sale for this venda
      ...
      if (resend && clienteData.email) {
        try {
          ...
          results.pos_venda_email++;
        } catch (e) {
          console.error('Pos-venda email error:', e);
        }
      }
    ...
    // Find clients whose last purchase was exactly ~30 days ago (window of 1 day)
    ...
    // Group by client_id → most recent purchase
    ...
    for (const envio of enviosRecentes || []) {
      ...
      // Also send email
      if (resend && envio.destinatario_telefone) {
        // Try to find email from ecommerce_pedidos or envios
        // We skip email here since envios table doesn't have email field
        // Email for shipping is handled separately when order status changes
      }
    }
    ...
    for (const pedido of pedidosEnviados || []) {
      ...
      if (resend) {
        try {
          ...
          console.error('Rastreio email error:', e);
        }
      }
    }
