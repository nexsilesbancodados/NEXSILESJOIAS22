...
export function SubscriptionNotifications() {
  const { assinatura, notificacoes, notificacoesNaoLidas, marcarNotificacaoLida } = useAssinatura();
  ...
  // Show trial activation if no subscription
  if (!assinatura) {
    return <TrialActivation />;
  }

  // Show subscription status + notifications
  return (
